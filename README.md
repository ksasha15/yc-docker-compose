...
Написать docker-compose для приложения с учётом требований предыдущих ДЗ.
В ReadMe вашего репозитория должна быть секция с описанием запуска docker-compose, в виде готовых команд.
Запустить ваше приложение на Yandex.Cloud через docker-compose.

Проверяются компетенции:
Работа с контейнерами и Docker
- работать с Docker-compose и сетевым взаиомдействием в Docker
...
drwxr-x--- 28 u24 u24 4096 Apr 18 10:13 ../
-rw-rw-r--  1 u24 u24  706 Apr 18 10:13 compose.yaml
-rw-rw-r--  1 u24 u24  244 Apr 18 10:11 Dockerfile
u24@Ubuntu22:~/yc-docker-compose$
root@Ubuntu22:~/yc-docker-compose$sudo docker compose up -d
root@Ubuntu22:~/yc-docker-compose$
```
#### Доступ на YandexCloud
Использован код для bastion-хоста из предыдущего задания. 
```
u24@Ubuntu22:~/yc-docker-compose$ ssh -i ../.ssh/ssh-key-1771855316608 yc-user@158.160.245.71
